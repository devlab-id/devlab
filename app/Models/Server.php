...
respond 404
                ...
                $conf =
                    "# This file is automatically generated by Devlab.\n".
    ...
    public function setupDynamicProxyConfiguration()
    {
        ...
        if ($this->proxyType() === ProxyTypes::TRAEFIK->value) {
            $file = "$dynamic_config_path/devlab.yaml";
            if (empty($settings->fqdn) || (isCloud() && $this->id !== 0) || ! $this->isLocalhost()) {
                ...
                if ($schema === 'https') {
                    $traefik_dynamic_conf['http']['routers']['devlab-http']['middlewares'] = [
                        0 => 'redirect-to-https',
                    ];

                    $traefik_dynamic_conf['http']['routers']['devlab-https'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'devlab',
                    ...
                    $traefik_dynamic_conf['http']['routers']['devlab-realtime-wss'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'devlab-realtime',
            ...
            $file = "$dynamic_config_path/devlab.caddy";
            if (empty($settings->fqdn) || (isCloud() && $this->id !== 0) || ! $this->isLocalhost()) {
                instant_remote_process([
                    "rm -f $file",
                ], $this);
...
$schema://$host {
    handle /app/* {
        reverse_proxy devlab-realtime:6001
    }
    reverse_proxy devlab:80
}";
    ...
    public function reloadCaddy()
    {
        return instant_remote_process([
            'docker exec devlab-proxy caddy reload --config /config/caddy/Caddyfile.autosave',
        ], $this);
    }
    ...
    public function checkSentinel()
    {
        ...
        if ($this->isSentinelEnabled()) {
            $sentinel_found = instant_remote_process(['docker inspect devlab-sentinel'], $this, false);
    ...
    public function getCpuMetrics(int $mins = 5)
    {
        if ($this->isMetricsEnabled()) {
            ...
            $cpu = instant_remote_process(["docker exec devlab-sentinel sh -c 'curl -H \"Authorization: Bearer {$this->settings->metrics_token}\" http://localhost:8888/api/cpu/history?from=$from'"], $this, false);
    ...
    public function getMemoryMetrics(int $mins = 5)
    {
        if ($this->isMetricsEnabled()) {
            $from = now()->subMinutes($mins)->toIso8601ZuluString();
            $memory = instant_remote_process(["docker exec devlab-sentinel sh -c 'curl -H \"Authorization: Bearer {$this->settings->metrics_token}\" http://localhost:8888/api/memory/history?from=$from'"], $this, false);
    ...
    public function getContainersWithSentinel(): Collection
    {
        ...
        $status = data_get($sentinel_found, '0.State.Status', 'exited');
        if ($status === 'running') {
            $containers = instant_remote_process(['docker exec devlab-sentinel sh -c "curl http://127.0.0.1:8888/api/containers"'], $this, false);
    ...
    public function loadUnmanagedContainers(): Collection
    {
        if ($this->isFunctional()) {
            ...
            $containers = $containers->map(function ($container) {
                ...
                if (! str($labels)->contains('devlab.managed')) {
                    return $container;
                }
    ...
    public function databases()
    {
        return $this->destinations()->map(function ($standaloneDocker) {
            ...
            return data_get($item, 'name') !== 'devlab-db';
        })->flatten();
    }
    ...
    public function validateDockerEngine($throwError = false)
    {
        ...
        $this->settings->save();
        $this->validateDevlabNetwork(isSwarm: false, isBuildServer: $this->settings->is_build_server);
    ...
    public function validateDockerSwarm()
    {
        ...
        $this->settings->is_usable = true;
        $this->settings->save();
        $this->validateDevlabNetwork(isSwarm: true);
    ...
    public function validateDevlabNetwork($isSwarm = false, $isBuildServer = false)
    {
        if ($isBuildServer) {
            return;
        }
        if ($isSwarm) {
            return instant_remote_process(['docker network create --attachable --driver overlay devlab-overlay >/dev/null 2>&1 || true'], $this, false);
        } else {
            return instant_remote_process(['docker network create devlab --attachable >/dev/null 2>&1 || true'], $this, false);
        }
    }
