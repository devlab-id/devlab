<x-layout>
    ...
    <div x-data="{ activeTab: window.location.hash ? window.location.hash.substring(1) : 'github' }">
        {{-- <div class="flex justify-center h-full gap-2 pb-6">
            <a class="flex items-center justify-center w-1/2 p-2 transition-colors rounded-none min-h-12 bg-devgray-200 hover:bg-devlab-100 hover:dark:text-white hover:no-underline"
                :class="activeTab === 'github' && 'bg-devlab dark:text-white'"
                @click.prevent="activeTab = 'github'; window.location.hash = 'github'" href="#">GitHub
            </a>
            <a class="flex items-center justify-center w-1/2 p-2 transition-colors rounded-none min-h-12 bg-devgray-200 hover:bg-devlab-100 hover:dark:text-white hover:no-underline"
                :class="activeTab === 'gitlab' && 'bg-devlab dark:text-white'"
